...
class GameProcessor:
    ...
    def process_game(self, game, odds_info, slot):
        ...
        pace_trend = self.engine.classify_pace_trend(state['total_samples']) if len(state['total_samples']) >= 5 else "Not enough data"
        
        logger.info(f"Projections - Home: {home_avg}, Away: {away_avg}, Total: {total_avg}")
        logger.info(f"Momentum - Home: {home_momentum}, Away: {away_momentum}")
        ...
        if quarter == 4 and not state['betting_window_fired']:
            ...
            state['betting_window_line'] = odds_info.get('totalLine') if odds_info else None
            
            # Recommendation logic
            if odds_info and 'totalLine' in odds_info:
                diff = total_avg - odds_info['totalLine']
            ...
            # Send betting decision alert
            self._send_betting_alert(home_name, away_name, home_score, away_score, total_score, total_avg, odds_info, state, slot, quarter, minute, second)
        ...
        if quarter >= 3 and not state['decision_window_complete']:
            ...
            self._send_projection_alert(home_name, away_name, home_score, away_score, total_score, total_avg, home_avg, away_avg, home_momentum, away_momentum, pace_trend, odds_info, slot, quarter, minute, second)
        
        state['last_timestamp'] = stamp
        self.state_mgr.save_state(event_id, state)
    
    def _send_betting_alert(self, home_name, away_name, home_score, away_score, total_score, total_avg, odds_info, state, slot, q, m, s):
...
🏁 **FINAL CALL: Place wager now if you are betting!**

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💰 **GAME TOTAL ({total_score})**

Avg: **{total_avg}** | Line: **{line}**
Diff: {'+' if diff > 0 else ''}{diff:.1f} pts | 🎯 **REC: {rec}**

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
        
        self.discord.send_message(message)
    
    def _send_projection_alert(self, home_name, away_name, home_score, away_score, total_score, total_avg, home_avg, away_avg, home_momentum, away_momentum, pace_trend, odds_info, slot, q, m, s):
        """Send projection alert to Discord"""
        line = odds_info.get('totalLine') if odds_info else 'N/A'
        diff = total_avg - line if isinstance(line, (int, float)) else 'N/A'
...
**{home_name}** ({home_score}) {self._format_momentum(home_momentum)}
Raw: {home_avg * 0.3:.1f} | Avg: **{home_avg}**

**{away_name}** ({away_score}) {self._format_momentum(away_momentum)}
Raw: {away_avg * 0.3:.1f} | Avg: **{away_avg}**

━━━━━━━━━━━━━━━━━━━━━━━━

💰 **GAME TOTAL ({total_score})**

Raw: {total_avg * 0.3:.1f} | Avg: **{total_avg}**
Line: **{line}** | Diff: {diff}

Samples: {len(state.get('total_samples', []))} | Reliability: {pace_trend}
